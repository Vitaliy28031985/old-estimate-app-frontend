...
import { useAddLowEstimateMutation } from '../../../redux/lowEstimate/lowEstimateApi';
import {projectsApi} from "../../../redux/projectSlice/projectSlice";
import 'react-toastify/dist/ReactToastify.css';
import s from "./AddEstimate.module.scss";

function AddLowEstimate({onModal}) {
    const {id} = useParams();
    const [title, setTitle] = useState('');
    const [addLowEstimate] = useAddLowEstimateMutation();
    ...
    const handleSubmit = async e => {
        ...
        try {
            const newEstimate = {id, estimates: { title: title || '' }}
                const add = await addLowEstimate(newEstimate);
